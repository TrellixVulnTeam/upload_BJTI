...
def delete_field(request):
    if request.method == "POST":
        user.objects(username= request.POST['your_name']).delete()
...
def update_table(request):
    if request.method == "POST":
        user.objects(username =  request.POST['old_name']).update(set__username = request.POST['new_name'])
...
def create_new_user(request):
    if request.method == "POST":
        if User.objects(username = request.POST['mail']) ==  None:

            user = User.create_user(request.POST['mail'], request.POST['pwd'])
            user.save()
            create_staff = False
            if create_staff:
                staff = User(username = "agatonvet", is_staff = True)
                staff.save()
